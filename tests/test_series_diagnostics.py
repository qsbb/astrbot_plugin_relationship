...
from astrbot_plugin_relationship.series_diagnostics import (
    diagnostic_clear,
    diagnostic_event,
    diagnostic_events,
    logger,
...
def test_series_diagnostics_are_bounded_redacted_and_isolated():
    ...
    diagnostic_event(
        "relationship.state",
        "updated 123456789",
        details={"changed": True, "platform_id": "123456789"},
    )
    logger.warning(
    ...
    assert payload["events"][0]["details"] == {"changed": True}
...
def test_series_diagnostics_cursor_and_capacity():
    ...
    for index in range(305):
        diagnostic_event("relationship.event", index)
