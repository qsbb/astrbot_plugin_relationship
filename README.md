...
`astrbot_plugin_relationship` v0.2.0 是凝心溯溪系列关系模块，统一管理 bot 对用户与会话的短期情绪、长期好感、信任和熟悉度，并输出结构化只读状态快照与行为建议。它不接管发送、不生成内容、不授予权限，也不执行跨插件动作。
...
普通有效互动微增，夸奖、得到帮助、被冒犯等事件按配置缓慢增减。每日正向与负向变化分别计额，避免正向刷满后吞掉负向事实（反之亦然）；长期无互动时向中位数缓慢回归。
